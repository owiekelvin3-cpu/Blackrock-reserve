...
const PREVIEW_YEARLY_TOTAL = 540323.45;
...
export default function CashFlowPanel({ data }: { data: CashFlowMonth[] }) {
  ...
  const cashFlowTotal = useMemo(() => {
    if (usingPreview) {
      return chartMode === "yearly"
        ? PREVIEW_YEARLY_TOTAL
        : (chartData[chartData.length - 1]?.inflow ?? 0) -
            (chartData[chartData.length - 1]?.outflow ?? 0);
    }
